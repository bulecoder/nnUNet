...
OUTPUT_CSV = os.path.join(SCRIPT_DIR, "4.bep_tee_length.csv")

# 3. 目标点的 ID (根据你的设定：3为BEP, 5为TEE)
BEP_ID = 3
TEE_ID = 5
# ============================================
...
def main():
    ...
    print(f"🚀 开始测量 BEP(ID:{BEP_ID}) 到 TEE(ID:{TEE_ID}) 的物理长度 (舌体长度)...")
    print("-" * 60)
    ...
    for current_dir in LABEL_DIRS:
        ...
        if not label_files:
            print(f"⚠️ 警告: 目录下未发现 .nii.gz 文件 -> {current_dir}")
            continue
        ...
        for filename in tqdm(label_files, leave=False):
            ...
            try:
                ...
                if p_bep is not None and p_tee is not None:
                    # 计算三维物理空间中的欧氏距离 (单位: mm)
                    length_mm = np.linalg.norm(p_bep - p_tee)
                else:
                    status = "Missing Landmark(s)"
                    if p_bep is None and p_tee is not None:
                        status = "Missing BEP"
                    elif p_tee is None and p_bep is not None:
                        status = "Missing TEE"
                
                results.append({
                    "Patient_Name": patient_name,
                    "BEP_TEE_Length_mm": length_mm,
                    "Status": status
                })
    ...
    # 统计信息打印
    valid_count = df['BEP_TEE_Length_mm'].notna().sum()
    print("-" * 60)
    print(f"🎉 测量任务全部完成！共扫描了 {total_processed_files} 个文件。")
    print(f"📊 成功测量双点距离的样本数: {valid_count} / {len(df)}")
    if valid_count > 0:
        mean_len = df['BEP_TEE_Length_mm'].mean()
        std_len = df['BEP_TEE_Length_mm'].std()
        print(f"📏 总体 BEP-TEE (舌体) 平均长度: {mean_len:.3f} ± {std_len:.3f} mm")
    print(f"📂 详细测量报告已保存至: \n   {OUTPUT_CSV}")
