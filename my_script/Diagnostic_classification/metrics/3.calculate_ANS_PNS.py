"""Calculate the three-dimensional ANS-PNS distance in physical space."""
...
OUTPUT_CSV = SCRIPT_DIR / "3.ans_pns_length.csv"
ANS_ID = 2
PNS_ID = 4


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Calculate ANS-PNS length (mm).")
...
def main() -> None:
    ...
    rows = []
    for case_id, path in tqdm(cases.items(), desc="3.ANS-PNS"):
        value = np.nan
        status = "Success"
        try:
            image = sitk.ReadImage(str(path))
            points = extract_landmark_centroids(image, [ANS_ID, PNS_ID])
            missing = missing_landmark_names(points, [ANS_ID, PNS_ID])
            if missing:
                status = f"Missing: {','.join(missing)}"
            else:
                value = float(np.linalg.norm(points[ANS_ID] - points[PNS_ID]))
        except Exception as exc:
            status = f"Error: {exc}"
        rows.append(
            {
                "Patient_Name": case_id,
                "ANS_PNS_Length_mm": value,
                "Status": status,
    ...
    valid = int(dataframe["ANS_PNS_Length_mm"].notna().sum())
    print(f"ANS-PNS completed: {valid}/{len(dataframe)} valid cases")
