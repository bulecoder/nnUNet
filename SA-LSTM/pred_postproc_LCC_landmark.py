...
"""
    ...
    (适配 Task 601 CBCT_Landmark)
"""

# ================= 配置区域 =================
TASK_ID = 601   # 修改为新的任务编号
DATASET_NAME = "CBCT_Landmark" # 修改为新的数据集名称
TASK_FOLD = 0   # 目标 fold，请根据你实际想要处理的 fold 修改 (例如 0, 1, 2, 3, 4)

# 1. 原始预测结果目录 (nnUNet 刚刚输出的文件夹，注意修改为你的 fold 目录)
# RAW_PRED_DIR = f"/data1/xyh/data/nnUNet/nnUNet_results/Dataset{TASK_ID}_{DATASET_NAME}/nnUNetTrainer__nnUNetPlans__3d_fullres/fold_{TASK_FOLD}/validation"

# 没有数据镜像的版本
RAW_PRED_DIR = f"/data1/xyh/data/nnUNet/nnUNet_results/Dataset{TASK_ID}_{DATASET_NAME}/nnUNetTrainerNoMirroring__nnUNetPlans__3d_fullres/fold_{TASK_FOLD}/validation"

# 2. 重绘后的最终输出目录
CLEAN_PRED_DIR = RAW_PRED_DIR + "_postprocessed_LCC"

# 3. 标签列表与物理半径 (已扩展至 12 个关键点)
